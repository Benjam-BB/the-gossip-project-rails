...
20.times do 	#cree 20 gossip en reference avec user
	Gossip.create(title: Faker::Lorem.sentence(word_count: 2), content: Faker::Quote.yoda, user: User.all.sample)
end
...
1.upto(20) do		#Fait la relation de 20 gossips avec des tags
	Gossip.all.each{|gsp| TagList.create(gossip: gsp, tag: Tag.all.sample)}	
end
